...
function RawkuOnboardingInterna({ usuario }) {
  ...
  const perfilYaCargado = useRef(false); // evita cargar dos veces si onAuthChange dispara varias veces

  useEffect(() => {
    if (!usuario) { setCargandoPerfil(false); return; }
    if (perfilYaCargado.current) return; // ya cargamos, ignorar segunda llamada
    perfilYaCargado.current = true;
    ...
    getPerros(usuario.id).then((perros) => {
      if (perros && perros.length > 0) {
        ...
        // Navegamos al generador DESPUÉS de que React aplique setPerfil
        setTimeout(() => {
          setFase("generador");
          setPantalla("elegir");
