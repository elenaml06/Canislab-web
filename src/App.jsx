...
function RawkuOnboardingInterna({ usuario }) {
  ...
  useEffect(() => {
    if (!usuario) { setCargandoPerfil(false); return; }

    // Cargar premium
    esPremium(usuario.id).then(setPremium).catch(() => setPremium(false));

    // Si viene de Stripe con pago ok
    ...
    // ─── CARGAR PERFIL DEL PERRO DESDE SUPABASE ──────────────
    getPerros(usuario.id).then((perros) => {
      console.log('[Rawku] perros en Supabase:', perros);
      if (perros && perros.length > 0) {
        const p = perros[0];
        const fechaNac = p.fecha_nacimiento ? new Date(p.fecha_nacimiento) : null;
        // Actualizamos todo de golpe en un solo batch para evitar
        // renders intermedios que muestren el onboarding un instante
        setPerfil((prev) => ({
          ...prev,
          _id: p.id,
          nombre: p.nombre || "",
          ...
          dia: fechaNac ? fechaNac.getDate() : prev.dia,
          mesIdx: fechaNac ? fechaNac.getMonth() : prev.mesIdx,
          anio: fechaNac ? fechaNac.getFullYear() : prev.anio,
          tamano: p.tamano,
          raza: p.raza ? { nombre: p.raza } : null,
          _perfilCargado: true, // flag para el render
        }));
        setFase("generador");
        setPantalla("elegir");
        console.log('[Rawku] perfil cargado, yendo al generador');
      } else {
        console.log('[Rawku] sin perros guardados, mostrando onboarding');
      }
    }).catch((e) => {
      console.error('[Rawku] error cargando perros:', e);
  ...
  // Mientras carga el perfil desde Supabase
  if (cargandoPerfil) {
    return (
      <div style={{ minHeight: '100dvh', background: '#FBF7FC', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
        ...
        <p style={{ color: '#9A8CB8', fontFamily: '"DM Sans", sans-serif', fontSize: 15 }}>Cargando tu perfil...</p>
      </div>
    );
  }

  // Si el perfil se cargó desde Supabase, ir al generador
  useEffect(() => {
    if (perfil._perfilCargado && fase === "onboarding") {
      setFase("generador");
      setPantalla("elegir");
    }
  }, [perfil._perfilCargado]);
