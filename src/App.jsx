...
function RawkuOnboardingInterna({ usuario }) {
  ...
  useEffect(() => {
    ...
    // Esto es lo que faltaba: al hacer login, buscamos si el usuario
    // ya tiene un perro guardado y lo cargamos directamente, saltando
    // el onboarding y yendo al generador.
    getPerros(usuario.id).then((perros) => {
      if (perros && perros.length > 0) {
        const p = perros[0]; // primer perro (multi-perro viene después)
        const fechaNac = p.fecha_nacimiento ? new Date(p.fecha_nacimiento) : null;
        setPerfil((prev) => ({
          ...prev,
          _id: p.id, // guardamos el ID para hacer UPDATE en vez de INSERT
          nombre: p.nombre || "",
    ...
    }).catch(console.error).finally(() => setCargandoPerfil(false));
  }, [usuario]);
