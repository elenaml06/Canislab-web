...
function RawkuOnboardingInterna() {
  // ─── AUTH ──────────────────────────────────────────────────
  const [usuario, setUsuario] = useState(undefined); // undefined = cargando, null = no autenticado
  ...
  useEffect(() => {
    const { data: { subscription } } = onAuthChange(async (user) => {
      setUsuario(user);
      if (user) {
        const p = await esPremium(user.id).catch(() => false);
        setPremium(p);
      } else {
        setPremium(false);
      }
  ...
  // Mientras comprueba la sesión, pantalla en blanco
