...
function respuestaApiAMenu(data, derObjetivo) {
  // convierte la respuesta de /menu/v2 ({menu: {alimento: gramos}}) al
  // formato que espera VistaMenus.
  // ⚠️ CORREGIDO 5 agosto: /menu (el viejo) devolvía "gramos"; /menu/v2
  // (el motor nuevo) devuelve "menu". Sin este cambio, la pantalla se
  // habría quedado sin datos aunque el backend respondiera bien.
  const items = Object.entries(data.menu || data.gramos || {}).map(([alimento, gramos]) => {
...
export default function CanislabOnboarding() {
  ...
  useEffect(() => {
    ...
    // ⚠️ CONECTADO al motor nuevo (5 agosto). /menu/v2 en vez de /menu:
    // el motor decide QUÉ alimentos usar Y cuánto de cada uno a la vez,
    // comprobado de forma exacta contra los 30 requisitos de FEDIAF, con
    // máximo 2 suplementos. Los tres modos (automático/personalizar/
    // aprovechar) ya existían en la app -- solo faltaba mandarlos.
    const nombresParaPersonalizar = Object.values(configPersonalizar)
      .filter((c) => c.modo === "manual" && c.elegido)
      .map((c) => c.elegido);
    ...
    const pedirMenu = () =>
      fetch(`${API_BASE}/menu/v2`, {
        ...
        body: JSON.stringify({
          ...
          nombres_alimentos:
            modo === "aprovechar" ? itemsAprovechar :
            modo === "personalizar" ? nombresParaPersonalizar :
            CANDIDATOS_AUTOMATICO_DEFECTO,
          forzar_presencia: modo === "personalizar" ? nombresParaPersonalizar : null,
          ...
          peso_perro_kg: Number(perfil.pesoActual) || null,
        }),
      }).then((res) => res.json());
